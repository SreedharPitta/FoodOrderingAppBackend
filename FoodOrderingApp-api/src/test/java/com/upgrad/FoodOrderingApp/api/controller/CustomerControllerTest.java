...
public class CustomerControllerTest {
    ...
    public void shouldUpdateCustomerPassword() throws Exception {
        ...
        when(mockCustomerService.getCustomer("auth")).thenReturn(customerEntity);
        when(mockCustomerService.updateCustomerPassword("oldPwd", "newPwd@123", customerEntity))
        ...
        mockMvc
                ...
                .andExpect(jsonPath("id").value(customerId));
        verify(mockCustomerService, times(1)).getCustomer("auth");
        verify(mockCustomerService, times(1)).updateCustomerPassword("oldPwd", "newPwd@123", customerEntity);
    }
    ...
    @Test
    public void shouldNotUpdateCustomerPasswordIfOldPasswordIsIncorrect() throws Exception {
        final CustomerEntity customerEntity = new CustomerEntity();
        when(mockCustomerService.getCustomer("auth")).thenReturn(customerEntity);
        when(mockCustomerService.updateCustomerPassword("oldPwd123", "newPwd@123", customerEntity))
                .thenThrow(new UpdateCustomerException("UCR-004", "Incorrect old password!"));
        mockMvc
                .perform(put("/customer/password")
                        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
                        .header("authorization", "Bearer auth")
                        .content("{\"old_password\":\"oldPwd123\", \"new_password\":\"newPwd@123\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("code").value("UCR-004"));
        verify(mockCustomerService, times(1)).getCustomer("auth");
        verify(mockCustomerService, times(1)).updateCustomerPassword("oldPwd123", "newPwd@123", customerEntity);
    }
